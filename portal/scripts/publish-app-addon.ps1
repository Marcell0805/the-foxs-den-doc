param(
    ...
    [string]$Channel = "live",
    [string]$ReleaseNotes = "Browser add-on update.",
...
if ($app.kind -ne 'addon') {
    throw "App '$AppId' kind is '$($app.kind)' - expected 'addon'"
}
...
$packageFileName = if ($app.packageFileName) { $app.packageFileName } else { "$AppId.zip" }
...
$versionPath = Join-Path $versionDir "addon-version.json"
...
if (Test-Path $iconScript) {
    ...
    & $iconScript -AppId $AppId -PortalRoot $PortalRoot -SourceRoot $repoPath -Kind "addon" -Quiet
}
